...
## ownership 원칙 (매 반복 적용 — 사람 개입 없는 루프이므로 특히 중요)

- 이번 커밋을 한 문장으로 설명할 수 없으면 커밋하지 않는다 (설명할 수 없다면 출시하지 마라)
- verify 실패 원인을 그대로 다음 반복에 복붙하지 말고, 원인을 스스로 재구성해서 수정한다 (인지적 굴복 금지)
